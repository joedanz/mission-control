...
mc workflow run <slug> [--timeout <sec>] [--allow-concurrent]   # run a workflow now, SYNCHRONOUSLY (keep prompts short — durable async execution lands in a later slice). Walks the graph: a trigger completes immediately; an agent node opens a real run (mc run start) and spawns the resolved profile via the shared daemon runner. RUN-ONLY visibility: cost/heartbeat/fleet-feed/cancel come from that run row — NO claimable task is created (so the auto-claim daemon can't race it). The graph is snapshotted onto the run; a single-flight guard refuses a second concurrent run unless --allow-concurrent
mc workflow status <runId>                   # a workflow run + its per-node step rows (each agent step links its runs id + captured output)
mc workflow cancel <runId>                   # request cancellation of a workflow run; propagates to the in-flight agent node's run (kill-switch)
