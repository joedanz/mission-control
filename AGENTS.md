...
mc workflow create --project <slug> --name <n> [--slug <s>] [--graph <json|@file>] [--description <d>]   # create a workflow from a React Flow {nodes,edges} graph. A provided graph is validated (DAG, exactly one trigger node, agent nodes need a prompt); omit --graph for an empty draft. Slug defaults to the slugified name (globally unique). Node types: trigger | agent | integration | branch | gate (slice 1 runs trigger + agent). An agent node's data = {prompt (required), profileSlug?, projectSlug?, responseSchema?}
